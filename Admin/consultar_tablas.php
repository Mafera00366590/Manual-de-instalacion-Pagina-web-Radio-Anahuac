...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($result->num_rows > 0) {
        // Mostrar los resultados de las tablas seleccionadas
        while ($row = $result->fetch_assoc()) {
            $table_name = $row["Tables_in_" . $dbname];
            // Verificar si la tabla está en la lista de tablas permitidas
            if (in_array($table_name, ['horario', 'programas', 'locutores', 'parrilla'])) {
                ...
                if ($table_data_result->num_rows > 0) {
                    ...
                    while ($table_header = $table_data_result->fetch_field()) {
                        echo "<th>" . htmlspecialchars($table_header->name) . "</th>";
                    }
                    echo "</tr>";
